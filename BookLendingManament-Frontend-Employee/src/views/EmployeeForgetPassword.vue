...
export default {
    ...
    methods: {
        async updatePass() {
            try {
                if (!this.username || !this.phoneNumber || !this.newPass){
                    this.message = "Vui lòng điền đủ thông tin";
                    return;
                }
                const resultUpdate = await employeeService.forgetPass({username: this.username, phoneNumber: this.phoneNumber, newPass: this.newPass});
